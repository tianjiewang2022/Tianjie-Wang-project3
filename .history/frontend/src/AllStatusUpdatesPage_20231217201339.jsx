...
export default function AllStatusUpdatesPage() {
    ...
    async function fetchAllStatusUpdates() {
        try {
            const response = await axios.get('/api/statusUpdatesWithUsers'); // Updated endpoint

            // Sort the updates based on the timestamp from newest to oldest
            const sortedUpdates = response.data.sort((a, b) => new Date(b.statusUpdate.timestamp) - new Date(a.statusUpdate.timestamp));

            setAllStatusUpdates(sortedUpdates);
            console.log(sortedUpdates);
        } catch (error) {
            console.error('Error fetching all status updates with users:', error);
        }
    }
    ...
    return (
        <div>
            <h2>All Status Updates</h2>
            {allStatusUpdates.map(({ user, statusUpdate }) => (
                <div key={statusUpdate._id}>
                    {/* Use Link to make the username clickable */}
